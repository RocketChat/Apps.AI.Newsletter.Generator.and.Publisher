...
export class ExecuteBlockActionHandler {
	...
	constructor(
		protected readonly app: NewsletterApp,
	...
	public async handleActions(): Promise<IUIKitResponse> {
		...
		switch (actionId) {
			case Modals.CONFIGURE_ACTION: {
				...
				let selected_llm = '';
				const LLMassociation = new RocketChatAssociationRecord(
					RocketChatAssociationModel.MISC,
					`${user.id}#selected_llm`
				);
				...
				if (selected_language != '' && selected_llm != '') {
					await sendNotification(
						...
						user,
						room,
						`You have successfully configured language: ` +
							selected_language +
							' with LLM: ' +
							selected_llm +
							` for code generation!`
			...
			case Modals.SHARE_ACTION: {
				const persis = this.read.getPersistenceReader();
				const association_input = new RocketChatAssociationRecord(
					RocketChatAssociationModel.MISC,
					`${user.id}#share_input`
				);
				...
				if (share_record) {
					...
					await sendMessage(this.modify, room, user, input_str);
				} else {
			...
			case Modals.REGEN_ACTION: {
				...
				if (result_record && regen_record) {
					...
					result_str = result_str.substring(
						0,
						Math.min(3000, result_str.length)
					);
