...
export class Handler {
	...
	public async regenerateCodeFromResult(dialogue: string, last_result: string) {
		...
		await sendNotification(
			...
			`You are using language: ` +
				...
				' with LLM: ' +
				this.LLM +
				` to refine code. Please wait for the response...
	...
	public async generateCodeFromParam(query: string) {
		...
		await sendNotification(
			...
			`You are using language: ` +
				...
				' with LLM: ' +
				this.LLM +
				` to generate code. Please wait for the response...
            (Please set language and LLM properly, otherwise you will not get any response!)`
		);
	...
	private async sendCodeResultwithBlocks(result: string) {
		const regen_block = await regenerationComponent(
			this.app,
			this.sender,
			this.read,
			this.persistence,
		...
		const share_block = await shareComponent(
			this.app,
			this.sender,
			this.read,
			this.persistence,
